...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      // If QPDF is not installed, return a clear error message instead of crashing
      if (execError.message.includes('not recognized') || execError.code === 127 || execError.message.includes('ENOENT')) {
        return NextResponse.json({ 
          success: false, 
          error: 'Server Error: qpdf is not installed on the server. Please install qpdf to use password encryption.' 
        }, { status: 501 });
      }
